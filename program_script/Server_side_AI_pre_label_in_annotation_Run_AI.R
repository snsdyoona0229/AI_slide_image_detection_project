    ...
    if (input$Slide_or_image == "Slide" & train_length > 0) {
     ...
     for(i in 1:length(img_location)){
	  ...
	  if(str_detect(image_name, ".ndpi")==TRUE){
		...
		thumb_image <- NDPI_center_RGB_batch_Num_function_Background(opened_slide,image_name,img_location[1],1024 * 2^(12 - 5),1024 * 2^(12 - 5),5,2048,slide_info_file)
	  }
	  #----#
        ...
        if (modified_label_table$Use_in_label[i] == "Y") {
          ...
          if (modified_label_table$Inference_H[i] != "-") {
			...
			for(ii in 1:length(all_heat_df)){
			 if(length(all_heat_df[[ii]]) >= 3){
			   if(length(all_heat_df[[ii]][[2]]) >0 & length(all_heat_df[[ii]][[3]]) > 0 & (NA %in% c(all_heat_df[[ii]][[2]],all_heat_df[[ii]][[3]]))==FALSE){
