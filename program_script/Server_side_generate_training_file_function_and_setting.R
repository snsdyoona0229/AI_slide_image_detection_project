...
observeEvent(input$Re_train_model_group, {
  updateSelectInput(session = session, inputId = "Re_train_model_generation", choices = list.files(paste0("model/", input$Re_train_model_group, "/")) %>% str_sort(numeric = TRUE))
})
    ...
    if (input$Training_Slide_or_image == "Image") {
      ...
      if (input$turn_on_point_label_on_train | input$turn_on_polygon_label_on_train) {
        ...
        for (i in 1:length(loop_ndpi_file)) {
          ...
          #saveRDS(list(
          #  loop_jpg_file = loop_ndpi_file[i],
          #  use_point_df = use_point_df,
          #  use_polygon_df = use_polygon_df,
          #  enhancement_point_df = enhancement_point_df,
          #  enhancement_polygon_df = enhancement_polygon_df,
        ...
        info_list[["new_or_retrain"]] <- input$New_train_or_Train_on_existed_model
        info_list[["retrain_group"]] <- input$Re_train_model_group
        info_list[["retrain_generation"]] <- input$Re_train_model_generation
        info_list[["retrain_type"]] <- input$Re_train_model_type
        info_list[["retrain_condition"]] <- input$Train_on_existed_model_condition
        ...
        file.copy(paste0("network_database/",input$training_network_class,"/Get_pre_load_function/Polygon_Function_train.R"), 
                  paste0(file_location_name, "/Polygon_Function_train.R"))		
...
output$polygon_label_on_train_table <- renderRHandsontable({
  ...
  if (file.exists(file_name)) {
    ...
    polygons_data <- polygons_data %>% 
      ...
      filter(Polygon_number != "") %>% 
      arrange(Primary_polygon_class, Secondary_polygon_class, Tertiary_polygon_class, Polygon_number)
