...
// [[Rcpp::export]]
RawVector NDPI_tile_call(std::string file_name, int x, int y, int z) {
  ...
  if (access_file != file_name & access_file != "start") {
	...
	ndprSetRequestString(nRequestID, "format", "BGR");
	ndprSetRequestInt(nRequestID, "imageid", nImageID);
	...
	UINT8* p_Value = (UINT8*)pBuffer;
	RawVector return_data(nSize);
		  
	for(int i=0; i<nSize; i++) {
	  return_data[i] = p_Value[i];
	}
	return return_data;
