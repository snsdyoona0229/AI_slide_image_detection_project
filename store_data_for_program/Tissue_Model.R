get_tissue <- function(input_shape = list(NULL, NULL, 3),
                     num_classes) {
  ...
  RS1L2u <- RS1L2 %>%
    layer_conv_2d_transpose(filters = 16, kernel_size = list(3, 3)) %>% 
  ...
  RS1L3u <- RS1L3 %>%
    layer_conv_2d_transpose(filters = 32, kernel_size = list(3, 3)) %>% 
  ...
  RS1L4u <- RS1L4 %>%
    layer_conv_2d_transpose(filters = 64, kernel_size = list(3, 3)) %>% 
  ...
  RS1L5u <- RS1L5 %>%
    layer_conv_2d_transpose(filters = 128, kernel_size = list(3, 3)) %>% 
  ...
  RS1L6u <- RS1L6 %>%
    layer_conv_2d_transpose(filters = 256, kernel_size = list(3, 3)) %>% 
  ...
  S1cu <- S1c %>%
    layer_conv_2d_transpose(filters = 512, kernel_size = list(3, 3)) %>% 
